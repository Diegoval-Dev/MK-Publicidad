...
const Finished = () => {
  ...
  // Función para obtener las cotizaciones finalizadas
  async function getFinishedQuotes() {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/quote/finalizado`, {
        method: 'GET',
        headers: {
  ...
  return (
    <div className='py-8'>
      {quotes.length === 0 ? (
        <p>No hay cotizaciones finalizadas</p>
      ) : (
